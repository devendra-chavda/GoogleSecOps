...
class GoogleSecOpsToStorage:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            applogger.exception("%s: API error", consts.LOG_PREFIX)

        applogger.info(
            "%s (%s): complete (responses saved=%d)",
            consts.LOG_PREFIX,
            method,
            batch_count,
        )
    ...
    def _write_response_to_file(self, response: dict, index: int) -> None:
        ...
        applogger.info(
            "%s: response saved → file=%s size=%.2f KB",
            consts.LOG_PREFIX,
            filename,
            size_kb,
        )
    ...
    def _update_checkpoint(
        ...
        if next_start:
            ...
            applogger.info(
                "%s: window complete, checkpoint advanced to %s",
                consts.LOG_PREFIX,
                next_start,
            )
        elif next_token:
            ...
            applogger.info(
                "%s: mid-window, checkpoint saved with token",
                consts.LOG_PREFIX,
            )
